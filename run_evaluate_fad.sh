...
#SBATCH --job-name=eval_fad
#SBATCH --nodes=1
#SBATCH --output=/work/vita/alefevre/programs/SonicMaster/logs/evaluate_fad/%j.out
#SBATCH --error=/work/vita/alefevre/programs/SonicMaster/logs/evaluate_fad/%j.err
...
if [ -z "$DEGRADATION" ]; then
  ...
  echo "Usage: sbatch run_evaluate_fad.sh <degradation_type> [eval_type]"
  echo "  eval_type: degraded (default), reconstructed, restored, restored_prompt"
  echo "  Example: sbatch run_evaluate_fad.sh airy restored"
  exit 1
fi

echo "Running FAD evaluation for degradation: $DEGRADATION"
...
echo "========================================="
echo "Running FAD evaluation..."
echo "========================================="
python evaluation/extract_fad_mass.py --jsonref "$INPUT_JSONL" --audio_key "$AUDIO_KEY"

echo ""
echo "✅ FAD evaluation complete!"
