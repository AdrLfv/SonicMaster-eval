...
#SBATCH --job-name=eval_kl_ssim
#SBATCH --nodes=1
#SBATCH --output=/work/vita/alefevre/programs/SonicMaster/logs/evaluate_kl_ssim/%j.out
#SBATCH --error=/work/vita/alefevre/programs/SonicMaster/logs/evaluate_kl_ssim/%j.err
...
if [ -z "$DEGRADATION" ]; then
  ...
  echo "Usage: sbatch run_evaluate_kl_ssim.sh <degradation_type> [eval_type]"
  echo "  eval_type: degraded (default), reconstructed, restored, restored_prompt"
  echo "  Example: sbatch run_evaluate_kl_ssim.sh airy restored"
  exit 1
fi

echo "Running KL-SSIM evaluation for degradation: $DEGRADATION"
...
echo "========================================="
echo "Running KL-SSIM evaluation..."
echo "========================================="
python evaluation/extract_kl_ssim_mass.py --jsonref "$INPUT_JSONL" --audio_key "$AUDIO_KEY"

echo ""
echo "✅ KL-SSIM evaluation complete!"
